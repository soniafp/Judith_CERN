...
LoopAlignCorr::LoopAlignCorr(const std::vector<Mechanics::Device*>& devices) :
    m_devices(devices) {}
...
void LoopAlignCorr::loop() {
  ...
  struct PlaneZ {
    ...
    unsigned iglobal;
    float z;
    PlaneZ() : idevice(0), iplane(0), iglobal(0), z(0) {}
    inline bool operator<(const PlaneZ& other) const { return z < other.offz; }
  };
  ...
  m_planeMap.assign(m_planesz.size(), 0);
  ...
  for (size_t i = 1; i < m_planesz.size(); i++)  // i iterates sorted in z
