...
class Plane:
    #100,445,545
    def __init__(self, x, z, pixelwidth=50.0, mask=False, npixel=12, allhits=False, tilt=0.0):
        ...
        self.tilt = tilt #0.261799388
        self.width = 300 # um

    def GetX(self, x_truth):
        return self.x
    
    def GetZ(self, x_truth):
        return self.z
...
for offset in [[0.0,-40.0, -1.0, 50.5, 12.0]]:
    ...
    if False:
        ...
        hists += [ROOT.TH1F('plane0','plane0',200,-500.0,500.0,tilt=0.261799388)]#0.261799388
        hists += [ROOT.TH1F('plane1','plane1',100,-100.0,100.0)]
        hists += [ROOT.TH1F('plane2','plane2',200,-500.0,500.0,tilt=-0.261799388)]#0.261799388
        if DUT:
            hists += [ROOT.TH1F('dut','dut',2000,-100.0,100.0)]        
        hists += [ROOT.TH1F('plane3','plane3',100,-100.0,100.0,tilt=0.261799388)]#0.261799388
        hists += [ROOT.TH1F('plane4','plane4',200,-500.0,500.0)]        
        hists += [ROOT.TH1F('plane5','plane5',100,-100.0,100.0,tilt=-0.261799388)]#0.261799388    
        if DUT:
        ...
        if DUT:
            telescope += [ Plane(dut_offset, 271.5, pixelwidth=50.0, mask=True, npixel=1)]        
        ...
        if DUT: # must match above plane
            telescope += [ Plane(dut_offset, 271.5, pixelwidth=50.0, mask=True, npixel=1, allhits=True)]                
