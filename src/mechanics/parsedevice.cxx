...
struct SensorBuff {
  ...
  double rotY;
  double rotZ;
  SensorBuff() : offX(0), offY(0), offZ(0), rotX(0), rotY(0), rotZ(0) {}
};
...
Device* parseDevice(const std::string& filePath) {
  ...
  enum Current { DEVICE, SENSOR, CHIP };
  Current current = DEVICE;  // start with the device

  std::ifstream file(filePath.c_str());
  if (!file.is_open())
    throw "Mechanics: parseDevice: unable to open file";
  ...
  while (std::getline(file, line)) {
    ...
    if (key == "sensor:") {
      ...
      else if (key == "space-unit") device.spaceUnit = value;
      ...
      if (key == "name") sensor.name = value;
      else if (key == "chip") sensor.chip = value;
      else if (key == "off-x") sensor.offX = strToFloat(value);
      else if (key == "off-y") sensor.offY = strToFloat(value);
      else if (key == "off-z") sensor.offZ = strToFloat(value);
      else if (key == "rot-x") sensor.rotX = strToFloat(value);
      else if (key == "rot-y") sensor.rotY = strToFloat(value);
      else if (key == "rot-z") sensor.rotZ = strToFloat(value);
      else throw std::runtime_error("Mechanics: parseDevice: unknown sensor key");
    }

    else if (current == CHIP) {
      ChipBuff& chip = chips.back();
      if (key == "name") chip.name = value;
      else if (key == "rows") chip.rows = strToInt(value);
  ...
  for (size_t i = 0; i < deviceObj->getNumSensors(); i++) {
    ...
    sensorObj.setOffX(sensor.offX);
    sensorObj.setOffY(sensor.offY);
    sensorObj.setOffZ(sensor.offZ);
    sensorObj.setRotX(sensor.rotX);
    sensorObj.setRotY(sensor.rotY);
    sensorObj.setRotZ(sensor.rotZ);
